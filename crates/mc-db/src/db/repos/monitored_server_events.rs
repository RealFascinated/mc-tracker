...
use crate::model::{MonitoredServerEvent, MonitoredServerEventType, Platform};

use super::{db_err, get_conn};

type EventRow = (Uuid, Uuid, String, String, String, DateTime<Utc>);
...
fn row_to_event(row: EventRow) -> Result<MonitoredServerEvent, DbError> {
    Ok(MonitoredServerEvent {
        ...
        server_id: row.1,
        server_name: row.2,
        server_type: Platform::from_db(&row.3).map_err(DbError::Database)?,
        event_type: MonitoredServerEventType::from_db(&row.4).map_err(DbError::Database)?,
        occurred_at: row.5,
    })
}
...
pub async fn insert(
    ...
    let row: EventRow = diesel::insert_into(monitored_server_events::table)
        .values((
            ...
            monitored_server_events::server_type.eq(event.server_type.as_str()),
        ...
        .returning((
            ...
            monitored_server_events::server_type,

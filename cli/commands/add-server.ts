  ...
  .action(
    async (
      ...
      try {
        ...
        const existsByIp = servers.some(
          (s: any) =>
            (s.ip || "").toString().toLowerCase() === host.toLowerCase(),
        );
        const existsByName = servers.some(
          (s: any) =>
        ...
        if (existsByIp || existsByName) {
          console.error(
            "Server with that name or host already exists in data/servers.json",
          );
          process.exit(1);
        }
          ...
          try {
            ...
            if (type && type.toUpperCase() === "PC") {
              ...
              try {
                ...
                console.log(
                  `Resolved ${host} to ${resolved.ip}:${resolved.port}`,
                );
            ...
            if (!pingResult) {
              console.error(
                `Server ${host}:${portNum} did not respond to ping`,
              );
              process.exit(1);
            }

            console.log(
              `✓ Server responded with ${pingResult.playerCount} player(s) online`,
            );
          } catch (err: any) {
            console.error(
              `Failed to ping server ${host}:${portNum}:`,

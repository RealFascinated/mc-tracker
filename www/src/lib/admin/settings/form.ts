...
function trimmedModelValues(models: LlmModelEntry[]): string[] {
...
function valuesEqual(
  ...
  if (field === "llmModels") {
    return (
      JSON.stringify(trimmedModelValues(loaded.llmModels)) ===
      JSON.stringify(trimmedModelValues(draft.llmModels))
    );
  }
...
export function dirtySettingPatches(
  ...
  >) {
    if (!valuesEqual(field, loaded, draft)) {
      const value =
        field === "llmModels"
          ? trimmedModelValues(draft.llmModels)
          : draft[field];
